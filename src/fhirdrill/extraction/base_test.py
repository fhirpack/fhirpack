...
def test_getResources_onlyInput(input, drillDocker, patientReference, patientResource):
...
def test_searchResources_patientId(searchParamsId, drillDocker):
...
@pt.mark.reqdocker
def test_getFromFiles_singleResource(drillDocker):

    d = drillDocker
    dataPath = (
        f"{ts.TEST_DATA_DIR}/fhirdrill.extraction.base.getFromFiles.patient.00.in"
    )
...
def test_getFromFiles_multipleResourcesFromOneFile(drillUnconnected):
...
def test_getFromFiles_multipleResourcesMultipleFiles(drillUnconnected):
...
def test_getFromFiles_multipleResourcesError(drillUnconnected):
    ...
    with pt.raises(TypeError):
        data = d.getFromFiles(
            [
                f"{ts.TEST_DATA_DIR}/fhirdrill.extraction.base.getFromFiles.patients.00.in",
                f"{ts.TEST_DATA_DIR}/fhirdrill.extraction.base.getFromFiles.observation.00.in",
            ]
        ).data
